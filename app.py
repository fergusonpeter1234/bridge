...
class BridgeGame:
    def __init__(self):
        self.players = {'South': None, 'North': None} # Human slots
        self.seats = ['South', 'West', 'North', 'East'] # West & East are Bots
        self.reset_game()
    ...
    def evaluate_bid(self, seat):
        ...
        if not self.bids or all(b['bid'] == 'PASS' for b in self.bids):
            ...
            if points < 12:
                ...
                return {'best': '1NT', 'reason': f"Balanced hand distribution with {points} HCP fits standard 1NT."}
            elif lengths['♠'] >= 5:
            ...
            return {'best': 'PASS', 'reason': "Game simplified responder system: Passing preserves board discipline."}
    ...
    def evaluate_card(self, seat):
        ...
        if not self.current_trick:
            # Prefer top of an honor sequence or highest card in longest suit
            best_card = max(hand, key=lambda c: c['val'])
            return best_card, f"Leading your highest card ({best_card['rank']}{best_card['suit']}) exerts trick control."
        ...
        if following_cards:
            highest_in_trick = max(self.current_trick, key=lambda p: p['card']['val'] if p['card']['suit'] == lead_suit else -1)
            ...
            if winning_moves:
                ...
                return best_card, f"Follow suit and cover the trick cheaply with {best_card['rank']}{best_card['suit']}."
            else:
                best_card = min(following_cards, key=lambda c: c['val'])
                return best_card, f"Cannot beat the current high card; duck low with {best_card['rank']}{best_card['suit']}."
        else:
            # Discard lowest card overall
            best_card = min(hand, key=lambda c: c['val'])
            return best_card, f"Void in {lead_suit}; discard low card ({best_card['rank']}{best_card['suit']}) to preserve high cards."
...
def on_bid(data):
    ...
    game.bids.append({'seat': seat, 'bid': bid, 'feedback': feedback, 'status': 'best' if is_best else 'okay'})

    # Check for bidding conclusion (3 passes or simple 4-bid cap)
    if len(game.bids) >= 4:
        game.phase = 'PLAY'
...
def on_play_card(data):
    ...
    game.hands[seat] = [c for c in game.hands[seat] if not (c['suit'] == played_card['suit'] and c['rank'] == played_card['rank'])]
    game.current_trick.append({'seat': seat, 'card': played_card, 'feedback': feedback, 'status': 'best' if is_best else 'okay'})

    # Trick completion (4 cards)
...
def send_game_state():
    ...
    if game.phase == 'BIDDING':
        ...
        advice = {'best': f"{best_card['rank']}{best_card['suit']}" if best_card else "None", 'reason': reason}
